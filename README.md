...
前端为 Web 管理界面（HTML 系统），由案件管理服务器提供，浏览器访问即可操作；本仓库脚本负责文书 OCR / 要素提取 / 数据写入，前后端配合使用。

![前台系统界面](docs/preview.png)
...
系统采用「服务器 + 客户端脚本」架构，**支持在局域网内部署，局域网内任意机器均可访问**：

- **服务器端**：案件管理系统（提供 Web 前台 + HTTP API，端口 8899）部署在公司局域网内的任一服务器上（Windows / Linux 均可）。部署完成后，局域网内任何电脑用浏览器访问 `http://服务器IP:8899` 即可打开前台 HTML 系统。
- **客户端脚本**：本仓库脚本可安装 / 拷贝到局域网内任意电脑，通过 `.env` 中的 `SERVER_URL` 指向服务器地址：

  ```ini
  SERVER_URL=http://192.168.1.100:8899   # 换成服务器实际的局域网 IP
  ```

- **多人协作**：局域网内多台电脑可同时运行脚本处理案件，数据统一写入服务器数据库，共享同一套案件数据。
- **安全提示**：默认使用 HTTP 明文传输，请仅在可信局域网内使用；如需跨公网访问，建议在服务器前加 HTTPS 反向代理。
